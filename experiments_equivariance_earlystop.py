...
import Models.SimpleNet as Models
...
def get_datasets(train_bs, val_bs, test_bs):
    ...
    train_dataset = datasets.MINST(root='../data', train=True, transform=image_transforms, download=True)

    train_set, val_set = torch.utils.data.random_split(train_dataset, [50000, 10000])

    test_dataset = datasets.MINST(root='./data', train=False, transform=image_transforms, download=True)
...
def fit(epochs, model, loss_func, opt, train_dl, valid_dl, classes, max_patience):
    path = "test_results/experiments_equivariance_earlystop/checkpoint-"+str(model)
    best_val_loss = 100.
...
def testing():
    ...
    create_test_dir("experiments_equivariance_earlystop")
    ...
    for model_to_test in models:
        ...
        with open("test_results/experiments_equivariance_earlystop/"+model_name+"-results.json", 'w') as outfile:
            outfile.write(json_object)

        torch.save(model.state_dict(), "test_results/experiments_equivariance_earlystop/"+model_name)
