...
def fit(epochs, model, loss_func, opt, train_dl, valid_dl, classes, max_patience):
    path = "test_results/MNIST/checkpoint-"+str(model)
    best_val_loss = 100.
...
def testing():
    ...
    for model_to_test in models:
        ...
        with open("test_results/MNIST/"+model_name+"-results.json", 'w') as outfile:
            outfile.write(json_object)

        torch.save(model.state_dict(), "test_results/MNIST/"+model_name)
